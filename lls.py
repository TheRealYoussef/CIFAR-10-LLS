...
class LLS(object):
    ...
    def train(self, data, labels):
        ...
        x = np.append(data, np.ones((n, 1), dtype=data.dtype), axis=1)
        inverse = inv(np.dot(x.T, x))
        self.w = np.empty((c, d + 1), dtype=np.float64)
        for i in range(c):
    ...
    def predict(self, data):
        # data is an n*d np array (n is the number of data points, d is the number of features)
        n = data.shape[0]
        y_pred = np.empty(n, dtype=self.labels_dtype)
        x = np.append(data.T, np.ones((1, n), dtype=data.dtype), axis=0)
        f = np.dot(self.w, x).T
